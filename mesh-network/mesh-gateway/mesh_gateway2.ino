...
uint32_t lastHB_painless = 0; // metric edit; tryna capture scale of painless mesh mesh time
...
void recieveMessageFromMesh(uint32_t from, String &msg) {
  ...
  // uint32_t sent = d["sent"]; // latency edit | verbosity for consistency
  // uint32_t recv = mesh.getNodeTime(); // latency edit | verbosity for consistency
  // uint32_t diff = recv - sent; // latency edit | verbosity for consistency
  Serial.printf("[rx] type=%s len=%u\n", type, msg.length());
  // Serial.printf("[rx] type=%s len=%u sent=%u received=%u delay(latency)=%u\n", type, msg.length(), d["sent"], recv, diff); // latency edit
  ...
  if (strcmp(type, "m") == 0) {
    ...
    uint32_t lat = recv - sent;
    Serial.printf("  ctl %s from %u\n", cmd, (unsigned)id);
    Serial.printf("  sent=%u recv=%u latency=%u", sent, recv, lat); // latency edit
...
void sendHeartBeat() {
  ...
  // edit 
  // mesh.startDelayMeas(); // putting this in heartbeat for now just to see how it works
  std::list<uint32_t> nodes = mesh.getNodeList();
  for (std::list<uint32_t>::iterator node = nodes.begin(); node != nodes.end(); node++) {
    // Serial.printf("Sending delay message to node %u", *node); // can be removed
    // mesh.startDelayMeas(*node); // can be removed
  ...
  lastHB = millis();
  lastHB_painless = mesh.getNodeTime(); // metric edit
}
...
void printStatus() {
  ...
  // Serial.printf("[painless compare] lastHB=%lu now=%lu latency(millis)=%lu latency(painless)=%lu\n", 
  //   (unsigned long)lastHB_painless, (unsigned long)mesh.getNodeTime(), (unsigned long)(millis() - lastHB), (unsigned long)(mesh.getNodeTime() - lastHB_painless)); 
  //   // metric edit; also ik this is a pain to read just too lazy to make variables ik ik
}
...
void setup() {
  ...
  mesh.onNodeDelayReceived([](auto nodeId, auto delay){ Serial.print("Delay: "); Serial.println(String(delay)); }); // edit FOR DELAY MEAS (can be removed)
