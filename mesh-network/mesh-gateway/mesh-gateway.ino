...
// Metric measuring stuff
uint32_t messages_rec = 0;
...
void electGatewayCheck() {
  ...
  if (newElection || (leaderId == 0)) { // temporary
...
void printStatus() {
  ...
  lastHB = millis(); // temporary
}

// Metric measuring stuff
void printThroughputRec() {
  Serial.printf("[%s] meshId=%u message throughput (received) = %u/s\n",
    nodeName.c_str(), meshId, messages_rec);
  messages_rec = 0; 
}

Task statusTask(1000, TASK_FOREVER, &printStatus);
Task sendHeartBeatThroughMesh(50, TASK_FOREVER, &sendHeartBeat);
Task electionTask(1000, TASK_FOREVER, &electGatewayCheck);

// Metric measuring stuff
Task throughputRecTask(1000, TASK_FOREVER, &printThroughputRec); // every second print and reset
...
void setup() {
  ...
  // Metric measuring stuff
  userScheduler.addTask(throughputRecTask);        throughputRecTask.enable();
